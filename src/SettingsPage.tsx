...
function SettingsPage({ onBack }: Props) {
  ...
  const destructiveAction = (action: 'scripts' | 'history' | 'account') => {
    ...
    // These require server-side authorization/deletion and should never pretend to succeed client-side.
  ...
  return <div className="settings-page-shell">
    <style>{`
      .settings-page-shell{min-height:100vh;padding:42px 20px 80px;color:var(--text)}
      ...
      .settings-page-back{display:inline-flex;align-items:center;gap:8px;padding:9px 12px;border:1px solid var(--border);border-radius:11px;background:var(--surface);color:var(--text);font-size:13px;font-weight:800;cursor:pointer}
      ...
      .settings-section{margin-top:18px;overflow:hidden;border:1px solid var(--border);border-radius:20px;background:var(--surface);box-shadow:var(--shadow)}
      .settings-section-heading{padding:19px 22px;border-bottom:1px solid var(--border)}
      .settings-section-heading h2{margin:0;color:var(--heading);font-size:16px}
      .settings-section-heading p{margin:5px 0 0;color:var(--muted);font-size:12px;line-height:1.5}
      .settings-setting{display:flex;align-items:center;justify-content:space-between;gap:25px;padding:19px 22px;border-bottom:1px solid var(--border)}
      ...
      .settings-danger{border-color:rgba(239,68,68,.45)}.settings-danger .settings-section-heading{border-bottom-color:rgba(239,68,68,.28)}.settings-danger .settings-section-heading h2{color:#ff6b6b}.settings-danger .settings-setting{border-bottom-color:rgba(239,68,68,.18)}.settings-danger-button{padding:10px 13px;border:1px solid rgba(239,68,68,.5);border-radius:11px;background:rgba(239,68,68,.08);color:#ff7777;font-size:12px;font-weight:900;cursor:pointer}.settings-danger-button:hover{background:rgba(239,68,68,.15);border-color:#ff6b6b}.settings-disabled{opacity:.55;cursor:not-allowed!important}
    ...
    <div className="settings-page-inner">
      ...
      {userEmail && <>
        ...
        <section className="settings-section settings-danger">
          ...
          <div className="settings-setting"><div className="settings-setting-copy"><strong>Delete all scripts</strong><span>Permanently remove scripts published by your account.</span></div><button className="settings-danger-button settings-disabled" onClick={() => setConfirming('scripts')} disabled>Delete scripts</button></div>
          ...
          <div className="settings-setting"><div className="settings-setting-copy"><strong>Delete account</strong><span>Permanently delete your Scriptly account and associated data.</span></div><button className="settings-danger-button settings-disabled" onClick={() => setConfirming('account')} disabled>Delete account</button></div>
        </section>
      </>}
