      ...
      <header className="navbar">
        ...
        <div className="nav-actions">
          ...
          <button className="profile-button" aria-label="Profile">
            ...
            <span className="profile-label">Sign in</span>
          </button>
        </div>
      </header>
      ...
      <main>
        <section className="hero-section">
          ...
          <div className="hero-actions">
            <button className="primary-button">Browse scripts</button>
            <button className="secondary-button">Publish a script</button>
          </div>
        </section>

        <section className="feed-header">
          <div><h2>Latest scripts</h2><p>Fresh code from the Scriptly community</p></div>
          <button className="filter-button">All languages <span>⌄</span></button>
        </section>
            ...
            return (
              <article className="script-card" key={script.title}>
                <div className="script-heading">
                  <div>
                    <div className="language"><span />{script.language}</div>
                    <h3>{script.title}</h3>
                    <p>{script.description}</p>
                  </div>
                  <button className="report-button" aria-label="Report script"><Icon name="flag" /></button>
                </div>
                <pre><code>{script.code}</code></pre>
                <div className="card-footer">
                  <div className="author"><span className="mini-avatar">{script.author[0].toUpperCase()}</span><span>{script.author}</span></div>
                  <div className="card-actions">
                    <button onClick={() => copyCode(script.code)}><Icon name="copy" /> Copy</button>
                    <button><Icon name="download" /> Download</button>
                    <button className={isLiked ? 'liked' : ''} onClick={() => setLiked((current) => ({ ...current, [script.title]: !isLiked }))}><Icon name="heart" /> {script.likes + (isLiked ? 1 : 0)}</button>
                  </div>
                </div>
