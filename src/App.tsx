...
const scripts: Script[] = [
  ...
  },
  {
    ...
    description: 'A clean CSS card that adapts smoothly from desktop to mobile.',
    language: 'CSS',
    author: 'pixeldev',
    likes: 17,
  ...
  },
  {
    ...
    description: 'A simple example using Python’s secrets module.',
    language: 'Python',
    author: 'noura',
    likes: 31,
...
function Icon({ name }: { name: 'search' | 'home' | 'settings' | 'user' | 'heart' | 'copy' | 'download' | 'flag' }) {
  const paths = {
    ...
    settings: <><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.9l.1.1-1.7 1.7-.1-.1a1.7 1.7 0 0 0-1.9-.3 1.7 1.7 0 0 0-1 1.6V22h-2.4v-.1a1.7 1.7 0 0 0-1-1.6 1.7 1.7 0 0 0-1.9.3l-.1.1L6 19l.1-.1a1.7 1.7 0 0 0 .3-1.9 1.7 1.7 0 0 0-1.6-1H4v-2.4h.8a1.7 1.7 0 0 0 1.6-1 1.7 1.7 0 0 0-.3-1.9L6 10.6l1.7-1.7.1.1a1.7 1.7 0 0 0 1.9.3l.1-.1 1.7 1.7-.1.1a1.7 1.7 0 0 0-.3 1.9 1.7 1.7 0 0 0 1.6 1h.1V16h-.6a1.7 1.7 0 0 0-1.6-1Z" /></>,
...
function App() {
  ...
  const visibleScripts = scripts.filter((script) =>
    `${script.title} ${script.description} ${script.language} ${script.author}`.toLowerCase().includes(query.toLowerCase()),
  )
  ...
  return (
    <div className={`app ${dark ? 'theme-dark' : 'theme-light'}`}>
      <header className="navbar">
        <button className="brand" onClick={() => setQuery('')} aria-label="Scriptly home">
          <span className="brand-mark">S</span>
          <span>Scriptly</span>
        </button>
        <div className="nav-center">
          <button className="nav-link active"><Icon name="home" /> <span>Home</span></button>
          <label className="search"><Icon name="search" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search scripts..." /><kbd>/</kbd></label>
        </div>
        <div className="nav-actions">
          <button className="icon-button" aria-label="Settings"><Icon name="settings" /></button>
          <button className="profile-button" aria-label="Profile"><span className="avatar"><Icon name="user" /></span><span className="profile-label">Sign in</span></button>
        </div>
      </header>
      <main>
        <section className="hero-section">
          <div className="eyebrow"><span className="pulse" /> COMMUNITY CODE LIBRARY</div>
          <h1>Find it. <span>Build it.</span> Share it.</h1>
          <p>Discover useful scripts, learn from other developers, and publish your own code.</p>
          <div className="hero-actions"><button className="primary-button">Browse scripts</button><button className="secondary-button">Publish a script</button></div>
        </section>
        ...
        <section className="script-grid">
          {visibleScripts.map((script) => {
            ...
            return <article className="script-card" key={script.title}>
              ...
              <div className="card-footer"><div className="author"><span className="mini-avatar">{script.author[0].toUpperCase()}</span><span>{script.author}</span></div><div className="card-actions"><button onClick={() => copyCode(script.code)}><Icon name="copy" /> Copy</button><button><Icon name="download" /> Download</button><button className={isLiked ? 'liked' : ''} onClick={() => setLiked((current) => ({ ...current, [script.title]: !isLiked }))}><Icon name="heart" /> {script.likes + (isLiked ? 1 : 0)}</button></div></div>
